...
function processMessage($message) {
  ...
  if (isset($message['text'])) {
    ...
    if (strpos($text, "/start") === 0) {
      apiRequestJson("sendMessage", array('chat_id' => $chat_id, "text" => "سلام به بازی XO(دوز)\nخوش اومدی برای شروع بازی روی دکمه اخر انتخاب کن و با دوستت بازی کن\nامیدوارم خوش باشی", 'reply_markup' => array(
...
function callbackMessage($callback){
	  ...
	  if(strpos($data, "play") === 0){
		  ...
		  apiRequest("editMessageText",array("inline_message_id"=>$message_id,"text"=>"کودوم تر سویی فرار کرد اگه نمیتونی بازی نکنی مرز داری رقابتو قبول میکنی"," reply_markup"=>array(
			"inline_keyboard"=>$Tab 
		  )));  
		  exit;
	  }
